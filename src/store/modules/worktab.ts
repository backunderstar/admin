...
export const useWorktabStore = defineStore(
  ...
  () => {
    ...
    const removeAll = (): void => {
      ...
      // TypeScript may consider targetTab possibly undefined; ensure a valid value
      current.value = targetTab || {}
    ...
    const toggleFixedTab = (path: string): void => {
      ...
      const targetTab = opened.value[targetIndex]
      if (!targetTab) return
      const tab = { ...targetTab }
    ...
    const validateWorktabs = (routerInstance: Router): void => {
      try {
        ...
        if (!isCurrentValid && validTabs.length > 0) {
          ...
          // validTabs.length > 0 ensures index 0 exists; spread to satisfy Partial<WorkTab> type
          current.value = { ...(validTabs[0] as WorkTab) }
