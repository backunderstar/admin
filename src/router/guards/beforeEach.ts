...
async function handleRouteGuard(
  ...
  // 1. 检查登录状态
  const loginStatus = handleLoginStatus(to, userStore)
  if (loginStatus !== true) {
    return loginStatus
  }
...
/**
 * 处理登录状态
 * @returns true 表示可以继续，RouteLocationRaw 表示需要跳转
 */
...
/**
 * 处理根路径重定向到首页
 * @returns false 表示无需跳转，RouteLocationRaw 表示需要重定向
 */
function handleRootPathRedirect(to: RouteLocationNormalized): false | RouteLocationRaw {
  if (to.path !== '/') {
    return false
  }
